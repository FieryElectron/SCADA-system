...
$.SvgCanvas = function(container, config) {
    ...
    var recalculateDimensions = this.recalculateDimensions = function(selected) {
        ...
        //!!!!!!!!!! not perfect!! for back ground, stroke
        // if(selected.tagName === "g"){
        // 	var AttrMap = parseDataAttribute($(selected).attr(Config.dataAttribute));
        // 	if(AttrMap){
        // 		var ctype = AttrMap.get("ctype");
        // 		if(ctype === "data" || ctype === "text"){
        // 			console.log(ctype);
        // 			return;
        // 		}
        // 	}
        ...
        var mouseMove = function(evt) {
            ...
            switch (current_mode) {
                ...
                case "resize":
                    ...
                    var keeprate = false;
                    if (selectedElements.length === 1) {
                        var group = selectedElements[0];
                        var AttrMap = root.parseDataAttribute($(group).attr(root.dataAttribute));
                        if (AttrMap) {
                            var ctype = AttrMap.get("ctype");
                            if (ctype && ctype === "gauge") {
                                keeprate = true;
                            }
                        }
                    }
                ...
                case "line":
                    ...
                    if (evt.shiftKey) { var xya = snapToAngle(start_x, start_y, x2, y2);
                        x2 = xya.x;
                        y2 = xya.y; }
                ...
                case "rotate":
                    ...
                    if (current_rotate_mode == "sw") { ccx = box.x + box.width;
            ...
            switch (current_mode) {
                ...
                case "select":
                    if (selectedElements[0] != null) {
                        ...
                        //!!!!!!!!!!
                        // var _data_ = selectedElements[0];
                        // if(_data_.tagName === "g"){
                        // 	var AttrMap = root.parseDataAttribute($(_data_).attr(root.dataAttribute));
                        // 	if(AttrMap){
                        // 		var ctype = AttrMap.get("ctype");
                ...
                if (current_mode === "path") {
                    ...
                    if (!drawn_path) {
                        ...
                        // else, create a new point, update path element
                            ...
                            if (evt.shiftKey) { var xya = snapToAngle(lastx, lasty, x, y);
                                x = xya.x;
                                y = xya.y; }
